...
function show_executing(s) {
    ...
    str += '--- RUN ---' + s + '\n';
    str += 'PC :' + PC + '\n';
    str += 'instr:' + util_1.getName(P[PC][INS_OPCODE_OFFSET]);
    return str;
}
...
M[opcodes_1.default.STRINGIFY] = () => {
    POP_OS();
    A = stringify_1.stringify(convertToJsFormat(RES));
    NEW_STRING();
    A = RES;
    PUSH_OS();
    PC = PC + 1;
...
function run() {
    ...
    while (RUNNING) {
        ...
        if (Date.now() - startTime > MAX_TIME) {
            throw new timeoutErrors_1.PotentialInfiniteLoopError(astCreator_1.locationDummyNode(-1, -1), MAX_TIME);
        }
        if (TO > 0) {
