...
test('Proper stringify-ing of arguments during potentially infinite iterative function calls', () => __awaiter(void 0, void 0, void 0, function* () {
  ...
  `;
    const error = yield testing_1.expectNativeToTimeoutAndError(code, 1000);
    expect(error).toMatch(formatters_1.stripIndent `
...
test('test increasing time limit for functions', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    f(1, 2);
  `;
    const firstError = yield testing_1.expectNativeToTimeoutAndError(code, 1000);
    expect(firstError).toMatch('Line 2: Potential infinite recursion detected');
    expect(firstError).toMatch(/f\(\d+, \d+\) \.\.\. f\(\d+, \d+\) \.\.\. f\(\d+, \d+\)/);
    const secondError = yield testing_1.expectNativeToTimeoutAndError(code, 10000);
...
test('test proper setting of variables in an outer scope', () => __awaiter(void 0, void 0, void 0, function* () {
    ...
    yield index_1.runInContext(formatters_1.stripIndent `
    let a = 'old';
    function f() {
      return a;
    }
  `, context);
    const result = yield index_1.runInContext('a = "new"; f();', context);
    expect(result.status).toBe('finished');
    expect(result.value).toBe('new');
}));
test('using internal names still work', () => __awaiter(void 0, void 0, void 0, function* () {
    const context = context_1.mockContext(3);
    let result = yield index_1.runInContext(formatters_1.stripIndent `
    const boolOrErr = 1;
    const program = 2;
    function wrap() {
      return boolOrErr;
  ...
  `, context);
    ...
    result = yield index_1.runInContext('program;', context);
    expect(result.status).toBe('finished');
    expect(result.value).toBe(2);
}));
test('assigning a = b where b was from a previous program call works', () => __awaiter(void 0, void 0, void 0, function* () {
    const context = context_1.mockContext(3);
    const result = yield index_1.runInContext(formatters_1.stripIndent `
    let b = null;
    b = pair;
    b = 1;
  `, context);
