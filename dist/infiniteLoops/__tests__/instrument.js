...
function runWithMock(main, codeHistory, builtins = new Map()) {
    ...
    const program = parser_1.parse(main, context);
    expect(program).not.toBeUndefined();
    let previous = [];
    if (codeHistory !== undefined) {
        const restOfCode = codeHistory.map(x => parser_1.parse(x, context));
    ...
    const instrumentedCode = instrument_1.instrument(previous, program, builtins.keys());
