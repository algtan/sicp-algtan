...
function typeCheck(program, context) {
    ...
    for (const code of context.unTypecheckedCode) {
        typeCheck_(parser_1.typedParse(code, context));
    }
        ...
        case 'MemberExpression': {
            ...
            if (envType.kind === 'predicate') {
                if (envType.ifTrueType.kind === 'forall') {
                    throw new internalTypeErrors_1.InternalTypeError(`Expected ${objName} to be an array, got ${stringify_1.typeToString(extractFreeVariablesAndGenFresh(envType.ifTrueType))}`);
                }
                else {
                    throw new internalTypeErrors_1.InternalTypeError(`Expected ${objName} to be an array, got ${stringify_1.typeToString(envType.ifTrueType)}`);
                }
            }
            else if (envType.kind === 'forall') {
                throw new internalTypeErrors_1.InternalTypeError(`Expected ${objName} to be an array, got ${stringify_1.typeToString(extractFreeVariablesAndGenFresh(envType))}`);
            }
            // ensure that the type is actually an array after unification
            const arrayType = applyConstraints(envType, newConstraints);
            if (arrayType.kind !== 'array')
                throw new internalTypeErrors_1.InternalTypeError(`Expected ${objName} to be an array, got ${stringify_1.typeToString(arrayType)}`);
