...
function prefixModule(program) {
    ...
    for (const node of program.body) {
        ...
        const moduleText = moduleLoader_1.memoizedGetModuleFile(node.source.value, 'bundle').trim();
...
function evallerReplacer(nativeStorageId, usedIdentifiers) {
    const arg = create.identifier(uniqueIds_1.getUniqueId(usedIdentifiers, 'program'));
...
function generateFunctionsToStringMap(program) {
    ...
    walkers_1.simple(program, {
        ArrowFunctionExpression(node) {
            map.set(node, astring_1.generate(node));
        },
...
function getNativeIds(program, usedIdentifiers) {
    ...
    for (const identifier of globalIdNames) {
        globalIds[identifier] = create.identifier(uniqueIds_1.getUniqueId(usedIdentifiers, identifier));
    }
    return globalIds;
}
    ...
    function instrumentLoops(node) {
        ...
        for (const statement of node.body) {
            if (statement.type === 'ForStatement' || statement.type === 'WhileStatement') {
                const startTimeConst = uniqueIds_1.getUniqueId(usedIdentifiers, 'startTime');
...
function transpileToSource(program, context, skipUndefined) {
    const usedIdentifiers = new Set([
        ...uniqueIds_1.getIdentifiersInProgram(program),
        ...uniqueIds_1.getIdentifiersInNativeStorage(context.nativeStorage)
    ]);
    ...
    const transpiled = modulePrefix + astring_1.generate(program, { sourceMap: map });
...
function transpileToFullJS(program) {
    ...
    const transpiled = astring_1.generate(transpiledProgram, { sourceMap });
    const sourceMapJson = sourceMap.toJSON();
    return { transpiled, sourceMapJson };
}
function transpile(program, context, skipUndefined = false) {
    if (runner_1.isFullJSChapter(context.chapter) || context.variant == 'native') {
