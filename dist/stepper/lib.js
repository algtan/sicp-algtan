...
function stringify(val) {
    return ast.literal(stepper_1.codify(val));
}
...
function is_number(val) {
    return ast.literal(util_1.isNumber(val));
}
...
function is_function(val) {
    return ast.literal(val.type.includes('Function') || util_1.isBuiltinFunction(val));
}
...
function parse_int(str, radix) {
    if (str.type === 'Literal' &&
        ...
        2 <= radix.value &&
        radix.value <= 36) {
        return converter_1.valueToExpression(parseInt(str.value, radix.value));
    }
    else {
...
function head(xs) {
    if (is_pair(xs).value === false) {
        throw new Error(`${stepper_1.codify(xs)} is not a pair`);
    }
