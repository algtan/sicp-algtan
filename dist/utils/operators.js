...
function wrapLazyCallee(candidate) {
    ...
    if (typeof candidate === 'function') {
        ...
        makeWrapper_1.makeWrapper(candidate, wrapped);
        ...
        if (candidate.evaluateArgs) {
            ...
            makeWrapper_1.makeWrapper(candidate.func, wrapped);
            wrapped[Symbol.toStringTag] = () => candidate.toString();
            wrapped.toString = () => candidate.toString();
            return wrapped;
        }
 ...
 */
exports.callIteratively = (f, nativeStorage, ...args) => {
    let line = -1;
    let column = -1;
    const startTime = Date.now();
    const pastCalls = [];
    while (true) {
        const dummy = astCreator_1.locationDummyNode(line, column);
        f = forceIt(f);
        if (typeof f === 'function') {
            ...
            if (hasVarArgs ? f.minArgsNeeded > receivedLength : expectedLength !== receivedLength) {
                throw new errors_1.InvalidNumberOfArguments(astCreator_1.callExpression(dummy, args, {
                    start: { line, column },
                    end: { line, column }
...
exports.wrap = (f, stringified, hasVarArgs, nativeStorage) => {
    if (hasVarArgs) {
        // @ts-ignore
        f.minArgsNeeded = f.length;
    }
    const wrapped = (...args) => exports.callIteratively(f, nativeStorage, ...args);
    makeWrapper_1.makeWrapper(f, wrapped);
    wrapped.transformedFunction = f;
    wrapped[Symbol.toStringTag] = () => stringified;
    wrapped.toString = () => stringified;
    return wrapped;
};
exports.setProp = (obj, prop, value, line, column) => {
    const dummy = astCreator_1.locationDummyNode(line, column);
...
exports.getProp = (obj, prop, line, column) => {
    const dummy = astCreator_1.locationDummyNode(line, column);
    const error = rttc.checkMemberAccess(dummy, obj, prop);
    if (error === undefined) {
