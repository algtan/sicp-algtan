...
function startRepl(chapter = 1, executionMethod = 'interpreter', variant = 'default', useSubst = false, useRepl, prelude = '') {
    // use defaults for everything
    const context = index_1.createContext(chapter, variant, undefined, undefined);
    const options = {
    ...
    index_1.runInContext(prelude, context, options).then(preludeResult => {
        if (preludeResult.status === 'finished' || preludeResult.status === 'suspended-non-det') {
            ...
            // the object being passed as argument fits the interface ReplOptions in the repl module.
            {
                eval: (cmd, unusedContext, unusedFilename, callback) => {
                    index_1.runInContext(cmd, context, options).then(obj => {
                        if (obj.status === 'finished' || obj.status === 'suspended-non-det') {
                            callback(null, obj.value);
                        }
                        else {
                            callback(new Error(index_1.parseError(context.errors)), undefined);
                        }
                    });
                },
            ...
            console.error(index_1.parseError(context.errors));
        }
    });
}
/**
