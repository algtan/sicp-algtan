...
test('parse_int with valid args is ok, radix 2', () => {
  ...
  `, { chapter: 1, native: true }).toBe(parseInt('1100101010101', 2));
});
test('parse_int with valid args is ok, radix 36', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    parse_int('uu1', 36);
  `, { chapter: 1, native: true }).toBe(parseInt('uu1', 36));
});
test('parse_int with valid args is ok, but invalid str for radix', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    parse_int('uu1', 2);
  `, { chapter: 1, native: true }).toBe(parseInt('uu1', 2));
});
...
test('arity with nullary function is ok', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    arity(math_random);
  `, { chapter: 1, native: true }).toBe(0);
});
test('arity with function with parameters is ok', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    arity(arity);
  `, { chapter: 1, native: true }).toBe(1);
});
...
test('arity with user-made function with rest parameter is ok', () => {
    ...
    arity(test);
  `, { chapter: 3, native: true }).toBe(0);
});
test('arity with non-function arg f throws error', () => {
    return testing_1.expectParsedError(formatters_1.stripIndent `
    arity('function');
  `, { chapter: 1, native: true }).toMatchInlineSnapshot(`"Line 1: Error: arity expects a function as argument"`);
});
