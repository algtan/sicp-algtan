...
function parseAt(source, num) {
    let theNode;
    try {
        theNode = acorn_1.parseExpressionAt(source, num, constants_1.ACORN_PARSE_OPTIONS);
    }
    catch (error) {
        return undefined;
    }
...
function parse(source, context) {
    let program;
    try {
        program = acorn_1.parse(source, exports.createAcornParserOptions(context));
        walkers_1.ancestor(program, walkers, undefined, context);
    }
    catch (error) {
...
function tokenize(source, context) {
    return [...acorn_1.tokenizer(source, exports.createAcornParserOptions(context))];
}
exports.tokenize = tokenize;
exports.createAcornParserOptions = (context) => ({
    sourceType: 'module',
    ecmaVersion: 6,
    locations: true,
    // tslint:disable-next-line:no-any
...
function parseForNames(source) {
    ...
    let program;
    try {
        program = acorn_1.parse(source, options);
    }
    catch (_a) {
        comments = [];
        program = acorn_loose_1.parse(source, options);
    }
    return [program, comments];
}
exports.parseForNames = parseForNames;
function looseParse(source, context) {
    const program = acorn_loose_1.parse(source, exports.createAcornParserOptions(context));
    return program;
}
...
function typedParse(code, context) {
    ...
    return validator_1.validateAndAnnotate(program, context);
}
