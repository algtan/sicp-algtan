...
--:
  Signifies the end of arguments, in case your input filename starts with -.`);
        ...
        const context = createContext_1.createEmptyContext(options.sourceChapter, options.sourceVariant, [], null);
        const program = parser_1.parse(source, context);
        let numWarnings = 0;
        let numErrors = 0;
        ...
        const vmInternalFunctions = options.sourceVariant === 'concurrent'
        ...
        const compiled = svml_compiler_1.compileToIns(program, undefined, vmInternalFunctions);
        if (options.compileTo === 'debug') {
            console.log(util_1.stringifyProgram(compiled).trimRight());
            return;
        }
        ...
        const binary = svml_assembler_1.assemble(compiled);
