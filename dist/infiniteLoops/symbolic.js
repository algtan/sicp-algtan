...
function hybridizeNamed(name, value) {
    if (isHybrid(value) || value === undefined || typeof value === 'function') {
        ...
        return exports.hybridValueConstructor(value, create.identifier(name));
    }
}
...
exports.hybridValueConstructor = (concrete, symbolic, validity = Validity.Valid) => ({
    type: 'value',
    concrete: concrete,
    symbolic: symbolic,
    validity: validity
...
exports.hybridArrayConstructor = (concrete, symbolic, listHeads = []) => ({
    type: 'array',
    concrete: concrete,
    symbolic: symbolic,
    listHeads: listHeads,
    ...
    function innerInplace(x) {
        ...
        for (let i = 0; i < x.length; i++) {
            if (Array.isArray(x[i])) {
                ...
                typeof x[i] === 'number') {
                x[i] = exports.hybridValueConstructor(x[i], create.identifier(`${name}'${count++}`));
            }
        }
    }
    innerInplace(concrete);
    // NOTE: below symbolic value won't be used in SMT
    return exports.hybridArrayConstructor(concrete, create.identifier(`${name}'array`));
}
...
function evaluateHybridBinary(op, lhs, rhs) {
    if (Array.isArray(shallowConcretize(lhs)) || Array.isArray(shallowConcretize(rhs))) {
        return exports.hybridValueConstructor(operators_1.evaluateBinaryExpression(op, shallowConcretize(lhs), shallowConcretize(rhs)), create.literal(false));
    }
    else if (isHybrid(lhs) || isHybrid(rhs)) {
        const val = operators_1.evaluateBinaryExpression(op, shallowConcretize(lhs), shallowConcretize(rhs));
        ...
        let res;
        if (op === '!==') {
            res = exports.hybridValueConstructor(val, neqRefine(lhs, rhs));
        }
        else {
            res = exports.hybridValueConstructor(val, create.binaryExpression(op, getAST(lhs), getAST(rhs)));
        }
        ...
        return operators_1.evaluateBinaryExpression(op, lhs, rhs);
    }
}
...
function evaluateHybridUnary(op, val) {
    if (isHybrid(val)) {
        const conc = operators_1.evaluateUnaryExpression(op, shallowConcretize(val));
        ...
        if (val.symbolic.type === 'Literal') {
            ...
            return exports.hybridValueConstructor(conc, newSym);
        }
        else if (op === '!' && val.type === 'value' && val.negation !== undefined) {
            const result = exports.hybridValueConstructor(conc, val.negation);
            result.negation = val.symbolic;
            return result;
        }
        else {
            return exports.hybridValueConstructor(conc, create.unaryExpression(op, getAST(val)));
        }
    }
    else {
        return operators_1.evaluateUnaryExpression(op, val);
    }
}
