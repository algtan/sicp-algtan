...
function __createKernelSource(end, externSource, localNames, arr, f, kernelId) {
    ...
    const ast = acorn_1.parse(code, constants_1.ACORN_PARSE_OPTIONS);
    const body = ast.body[0].expression;
    const newBody = transfomer_1.gpuRuntimeTranspile(body, new Set(localNames));
    const kernel = new Function(astring_1.generate(newBody));
