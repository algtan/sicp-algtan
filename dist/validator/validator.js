...
function validateAndAnnotate(program, context) {
    ...
    function processBlock(node) {
        ...
        for (const statement of node.body) {
            if (statement.type === 'VariableDeclaration') {
                initialisedIdentifiers.set(astCreator_1.getVariableDecarationName(statement), new Declaration(statement.kind === 'const'));
            }
    ...
    walkers_1.ancestor(program, {
        ...
        ForStatement(forStatement, ancestors) {
            ...
            if (init.type === 'VariableDeclaration') {
                accessedBeforeDeclarationMap.set(forStatement, new Map([[astCreator_1.getVariableDecarationName(init), new Declaration(init.kind === 'const')]]));
    ...
    walkers_1.ancestor(program, {
        VariableDeclaration(node, ancestors) {
            const lastAncestor = ancestors[ancestors.length - 2];
            const name = astCreator_1.getVariableDecarationName(node);
