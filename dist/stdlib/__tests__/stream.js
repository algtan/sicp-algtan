...
describe('primitive stream functions', () => {
    test('empty stream is null', () => {
        return testing_1.expectResult('stream();', { chapter: 3, native: true }).toBe(null);
    });
    test('stream_tail works', () => {
        return testing_1.expectResult(`head(stream_tail(stream(1, 2)));`, { chapter: 3, native: true }).toBe(2);
    });
...
`);
    ...
    `, { chapter: 3, native: true }).toMatch(/(Maximum call stack size exceeded){1,2}/);
    }, 15000);
    ...
    `, { chapter: 3, native: true }).toMatchInlineSnapshot(`false`);
    });
    test('stream_to_list works for null', () => {
        return testing_1.expectResult(`stream_to_list(null);`, {
            chapter: 3,
            native: true
        }).toMatchInlineSnapshot(`null`);
    });
    test('stream_to_list works', () => {
        return testing_1.expectResult(`stream_to_list(stream(1, true, 3, 4.4, [1, 2]));`, {
            chapter: 3,
            native: true
...
test('remove', () => {
  ...
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`null`);
});
...
test('list_ref', () => {
  ...
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`4`);
});
