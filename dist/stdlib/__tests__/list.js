...
test('head works', () => {
  ...
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`1`);
});
test('tail works', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    tail(pair(1, 'a string ""'));
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"a string \\"\\""`);
});
...
test('equal', () => {
  ...
  `, { chapter: 2 }).toMatchInlineSnapshot(`true`);
});
test('for_each', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    let sum = 0;
    ...
    sum;
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`6`);
});
test('map', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    equal(map(x => 2 * x, list(12, 11, 3)), list(24, 22, 6));
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`true`);
});
test('filter', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
...
test('list_ref', () => {
  ...
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`4`);
});
test('accumulate', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    accumulate((curr, acc) => curr + acc, 0, list(2, 3, 4, 1));
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`10`);
});
test('list_to_string', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    list_to_string(list(1, 2, 3));
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"[1,[2,[3,null]]]"`);
});
// assoc removed from Source
test.skip('assoc', () => {
    return testing_1.expectResult(formatters_1.stripIndent `
    equal(assoc(3, list(pair(1, 2), pair(3, 4))), pair(3, 4));
  `, { chapter: 100, native: true }).toMatchInlineSnapshot(`true`);
});
...
test('non-list error head', () => {
    ...
    head([1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
});
test('non-list error tail', () => {
    return testing_1.expectParsedError(formatters_1.stripIndent `
    tail([1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Error: tail(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
});
describe('These tests are reporting weird line numbers, as list functions are now implemented in Source.', () => {
    test('non-list error length', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    length([1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 33: Error: tail(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error map', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    map(x=>x, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 47: Error: tail(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error for_each', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    for_each(x=>x, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 76: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error reverse', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    reverse([1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 106: Error: tail(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error append', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    append([1, 2, 3], list(1, 2, 3));
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 121: Error: tail(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error member', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    member(1, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 136: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error remove', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    remove(1, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 151: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error remove_all', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    remove_all(1, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 169: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error assoc', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    assoc(1, [1, 2, 3]);
  `, { chapter: 100 }).toMatchInlineSnapshot(`"Line 1: Name assoc not declared."`);
    });
    test('non-list error filter', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    filter(x => true, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 185: Error: head(xs) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error accumulate', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    accumulate((x, y) => x + y, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Expected 3 arguments, but got 2."`);
    });
    test('non-list error accumulate', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    accumulate((x, y) => x + y, [1, 2, 3]);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Expected 3 arguments, but got 2."`);
    ...
    test('non-list error set_head', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    set_head([1, 2, 3], 4);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Error: set_head(xs,x) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    test('non-list error set_tail', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    set_tail([1, 2, 3], 4);
  `, { chapter: 3, native: true }).toMatchInlineSnapshot(`"Line 1: Error: set_tail(xs,x) expects a pair as argument xs, but encountered [1, 2, 3]"`);
    });
    // skipped as implementation does not check types, causing infinite recursion.
    test.skip('bad number error build_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    build_list(x => x, -1);
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 1: Error: build_list(fun, n) expects a positive integer as argument n, but encountered -1"`);
    });
    // skipped as implementation does not check types, causing infinite recursion.
    test.skip('bad number error build_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    build_list(x => x, 1.5);
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 1: Error: build_list(fun, n) expects a positive integer as argument n, but encountered 1.5"`);
    });
    test('bad number error build_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    build_list(x => x, '1');
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 63: Expected number on left hand side of operation, got string."`);
    });
    test('bad number error enum_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    enum_list('1', '5');
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 203: Expected string on right hand side of operation, got number."`);
    });
    test('bad number error enum_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    enum_list('1', 5);
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 201: Expected string on right hand side of operation, got number."`);
    });
    test('bad number error enum_list', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    enum_list(1, '5');
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 201: Expected number on right hand side of operation, got string."`);
    });
    test('bad index error list_ref', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    list_ref(list(1, 2, 3), 3);
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 216: Error: head(xs) expects a pair as argument xs, but encountered null"`);
    });
    test('bad index error list_ref', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    list_ref(list(1, 2, 3), -1);
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 217: Error: tail(xs) expects a pair as argument xs, but encountered null"`);
    });
    test('bad index error list_ref', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
  ...
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 217: Error: tail(xs) expects a pair as argument xs, but encountered null"`);
    ...
    test('bad index error list_ref', () => {
        return testing_1.expectParsedError(formatters_1.stripIndent `
    list_ref(list(1, 2, 3), '1');
  `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 215: Expected string on right hand side of operation, got number."`);
    });
});
describe('display_list', () => {
    ...
    test('checks prepend type', () => {
        ...
        0; // suppress long result in snapshot
      `, { chapter: 2, native: true }).toMatchInlineSnapshot(`"Line 1: TypeError: display_list expects the second argument to be a string"`);
    });
    /**************
     * FUZZ TESTS *
     **************/
