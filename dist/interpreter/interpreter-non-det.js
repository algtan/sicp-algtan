...
function* getAmbRArgs(context, call) {
    const args = lodash_1.cloneDeep(call.arguments);
    while (args.length > 0) {
...
exports.evaluators = {
    ...
    UnaryExpression: function* (node, context) {
        ...
        for (const argValue of argGenerator) {
            ...
            yield operators_1.evaluateUnaryExpression(node.operator, argValue);
        }
        return;
    },
    BinaryExpression: function* (node, context) {
        ...
        for (const pair of pairGenerator) {
            ...
            yield operators_1.evaluateBinaryExpression(node.operator, leftValue, rightValue);
        }
        return;
    },
        ...
        function* loop() {
            ...
            for (const test of testGenerator) {
                ...
                if (test &&
                    ...
                    const iterationValueGenerator = evaluate(lodash_1.cloneDeep(node.body), context);
...
function* apply(context, fun, args, node, thisContext) {
    if (fun instanceof closure_1.default) {
        ...
        pushEnvironment(context, environment);
        const applicationValueGenerator = evaluateBlockSatement(context, lodash_1.cloneDeep(fun.node.body));
