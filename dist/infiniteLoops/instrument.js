...
function hybridizeVariablesAndLiterals(program) {
    walkers_1.recursive(program, true, {
...
function trackIfStatements(program) {
    ...
    walkers_1.simple(program, { IfStatement: theFunction, ConditionalExpression: theFunction });
}
/**
...
function trackLocations(program) {
    ...
    walkers_1.recursive(program, undefined, {
        CallExpression(node, state, callback) {
            if (node.callee.type === 'MemberExpression')
                return;
...
function transformImportDeclarations(programs) {
    ...
    for (const program of programs) {
        ...
        for (const node of imports) {
            result = transpiler_1.transformSingleImportDeclaration(moduleCounter, node).concat(result);
            moduleCounter++;
        }
...
function instrument(previous, program, builtins) {
    ...
    const code = astring_1.generate(program);
    return prefix + code;
}
