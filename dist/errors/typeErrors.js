...
class InvalidArrayIndexType {
    ...
    explain() {
        return `Expected array index as number, got ${stringify_1.typeToString(this.receivedType)} instead`;
    }
    elaborate() {
        return this.explain();
    }
...
class ArrayAssignmentError {
    ...
    explain() {
        return formatters_1.stripIndent `Expected array type: ${stringify_1.typeToString(this.arrayType)}
    but got: ${stringify_1.typeToString(this.receivedType)}`;
    }
    elaborate() {
        return this.explain();
    }
...
class DifferentAssignmentError {
    ...
    to get a value of type:
      ${stringify_1.typeToString(this.expectedType)}
    but got a value of type:
      ${stringify_1.typeToString(this.receivedType)}
    `;
    }
    elaborate() {
        return this.explain();
...
function formatAssignment(node) {
    ...
    const assignmentStr = formatters_1.simplify(astring_1.generate(node.right));
...
class InvalidArgumentTypesError {
    ...
    explain() {
        const argStrings = this.args.map(arg => formatters_1.simplify(astring_1.generate(arg)));
        ...
        The binary operator (${op}) expected two operands with types:
          ${stringify_1.typeToString(this.expectedTypes[0])} ${op} ${stringify_1.typeToString(this.expectedTypes[1])}
        but instead it received two operands of types:
          ${stringify_1.typeToString(this.receivedTypes[0])} ${op} ${stringify_1.typeToString(this.receivedTypes[1])}
        `;
            }
            else {
                // unary operator
        ...
        The unary operator (${op}) expected its operand to be of type:
          ${stringify_1.typeToString(this.expectedTypes[0])}
        but instead it received an operand of type:
          ${stringify_1.typeToString(this.receivedTypes[0])}
        `;
            }
        }
        const functionString = formatters_1.simplify(astring_1.generate(this.node));
        function formatPhrasing(types) {
            switch (types.length) {
                // there will at least be one argument
                case 1:
                    return `an argument of type:
      ${stringify_1.typeToString(types[0])}`;
                default:
...
function formatNodeWithTest(node) {
    let exprString = formatters_1.simplify(astring_1.generate(node.test));
    let kind;
    switch (node.type) {
        case 'IfStatement': {
...
class ConsequentAlternateMismatchError {
    ...
    The true branch has type:
      ${stringify_1.typeToString(this.consequentType)}
    but the false branch has type:
      ${stringify_1.typeToString(this.alternateType)}
    `;
    }
    elaborate() {
        return this.explain();
...
class CallingNonFunctionType {
    ...
    explain() {
        return formatters_1.stripIndent `
    In
      ${formatters_1.simplify(astring_1.generate(this.node))}
    expected
      ${formatters_1.simplify(astring_1.generate(this.node.callee))}
    to be a function type, but instead it is type:
      ${stringify_1.typeToString(this.callerType)}
    `;
    }
    elaborate() {
        return this.explain();
...
class InconsistentPredicateTestError {
    ...
    explain() {
        const exprString = astring_1.generate(this.node);
        return formatters_1.stripIndent `
    ...
    The variable ${this.argVarName} has type
      ${stringify_1.typeToString(this.preUnifyType)}
    but could not unify with type
      ${stringify_1.typeToString(this.predicateType)}
    `;
    }
    elaborate() {
        return this.explain();
