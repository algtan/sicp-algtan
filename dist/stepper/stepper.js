...
function isIrreducible(node) {
    return (util_1.isBuiltinFunction(node) ||
        util_1.isAllowedLiterals(node) ||
        util_1.isNegNumber(node) ||
...
function findMain(target) {
    ...
    const finders = {
        ...
        BlockStatement(target) {
            seenBefore.set(target, target);
            const declaredNames = util_1.getDeclaredNames(target);
...
function substituteMain(name, replacement, target, paths) {
    ...
    const substituters = {
        ...
        ExpressionStatement(target, index) {
            const substedExpressionStatement = ast.expressionStatement(dummyAstCreator_1.dummyExpression());
        ...
        BinaryExpression(target, index) {
            const substedBinaryExpression = ast.binaryExpression(target.operator, dummyAstCreator_1.dummyExpression(), dummyAstCreator_1.dummyExpression(), target.loc);
        ...
        UnaryExpression(target, index) {
            const substedUnaryExpression = ast.unaryExpression(target.operator, dummyAstCreator_1.dummyExpression(), target.loc);
        ...
        ConditionalExpression(target, index) {
            const substedConditionalExpression = ast.conditionalExpression(dummyAstCreator_1.dummyExpression(), dummyAstCreator_1.dummyExpression(), dummyAstCreator_1.dummyExpression(), target.loc);
        ...
        CallExpression(target, index) {
            const dummyArgs = target.arguments.map(() => dummyAstCreator_1.dummyExpression());
            const substedCallExpression = ast.callExpression(dummyAstCreator_1.dummyExpression(), dummyArgs, target.loc);
        ...
        FunctionDeclaration(target, index) {
            ...
            const substedFunctionDeclaration = ast.functionDeclaration(newID, substedParams, dummyAstCreator_1.dummyBlockStatement());
        ...
        FunctionExpression(target, index) {
            ...
            const substedFunctionExpression = target.id
                ? ast.functionDeclarationExpression(target.id, substedParams, dummyAstCreator_1.dummyBlockStatement())
                : ast.functionExpression(substedParams, dummyAstCreator_1.dummyBlockStatement());
        ...
        Program(target, index) {
            const substedBody = target.body.map(() => dummyAstCreator_1.dummyStatement());
        ...
        ReturnStatement(target, index) {
            const substedReturnStatement = ast.returnStatement(dummyAstCreator_1.dummyExpression(), target.loc);
        ...
        ArrowFunctionExpression(target, index) {
            ...
            let newBody = target.body;
            const substedArrow = ast.arrowFunctionExpression(substedParams, dummyAstCreator_1.dummyBlockStatement());
        ...
        VariableDeclaration(target, index) {
            const substedVariableDeclaration = ast.variableDeclaration([dummyAstCreator_1.dummyVariableDeclarator()]);
        ...
        VariableDeclarator(target, index) {
            ...
            let substedVariableDeclarator = ast.variableDeclarator(subbed, dummyAstCreator_1.dummyExpression());
            ...
            if (target.id.type === 'Identifier' && name.name === target.id.name) {
                if (replacement.type == 'Identifier' && re.test(replacement.name)) {
                    ...
                    substedVariableDeclarator = ast.variableDeclarator(newName, dummyAstCreator_1.dummyExpression());
                }
        ...
        IfStatement(target, index) {
            const substedIfStatement = ast.ifStatement(dummyAstCreator_1.dummyExpression(), dummyAstCreator_1.dummyBlockStatement(), dummyAstCreator_1.dummyBlockStatement(), target.loc);
        ...
        ArrayExpression(target, index) {
            const substedArray = ast.arrayExpression([dummyAstCreator_1.dummyExpression()]);
...
function reduceMain(node, context) {
    ...
    const reducers = {
        ...
        Identifier(node, context, paths) {
            ...
            if (!(util_1.isAllowedLiterals(node) || util_1.isBuiltinFunction(node))) {
            ...
            if (isIrreducible(left)) {
                if (isIrreducible(right)) {
                    ...
                    if (builtin.is_function(left).value &&
                        ...
                        operator === '===') {
                        return [converter_1.valueToExpression(left === right), context, paths, explain(node)];
                    }
                    ...
                    if (error === undefined) {
                        const lit = operators_1.evaluateBinaryExpression(operator, leftValue, rightValue);
                        return [converter_1.valueToExpression(lit, context), context, paths, explain(node)];
                    }
                    else {
                        throw error;
                    }
        ...
        UnaryExpression(node, context, paths) {
            ...
            if (isIrreducible(argument)) {
                ...
                const argumentValue = converter_1.nodeToValue(argument);
                const error = rttc.checkUnaryExpression(node, operator, argumentValue, context.chapter);
                if (error === undefined) {
                    const result = operators_1.evaluateUnaryExpression(operator, argumentValue);
                    return [converter_1.valueToExpression(result, context), context, paths, explain(node)];
                }
                else {
                    throw error;
                }
            ...
            if (node.body.length === 0) {
                ...
                if (firstStatement.type === 'ReturnStatement') {
                    ...
                    if (kind !== 'const') {
                        ...
                        return [dummyAstCreator_1.dummyProgram(), context, paths, 'cannot use let or var'];
                    }
                        ...
                        !declarations[0].init) {
                        // TODO: syntax error
                        return [dummyAstCreator_1.dummyProgram(), context, paths, 'syntax error'];
                    }
                    else {
                        ...
                        if (declarator.id.type !== 'Identifier') {
                            ...
                            return [dummyAstCreator_1.dummyProgram(), context, paths, 'source does not allow destructuring'];
                        }
        ...
        BlockStatement(node, context, paths) {
            if (node.body.length === 0) {
                ...
                if (firstStatement.type === 'ReturnStatement') {
                    ...
                    if (kind !== 'const') {
                        // TODO: cannot use let or var
                        return [dummyAstCreator_1.dummyBlockStatement(), context, paths, 'cannot use let or var'];
                    }
                    else if (declarations.length <= 0 ||
                        declarations.length > 1 ||
                        declarations[0].type !== 'VariableDeclarator' ||
                        !declarations[0].init) {
                        // TODO: syntax error
                        return [dummyAstCreator_1.dummyBlockStatement(), context, paths, 'syntax error'];
                    }
                    else {
                        const declarator = declarations[0];
                        const rhs = declarator.init;
                        if (declarator.id.type !== 'Identifier') {
                            // TODO: source does not allow destructuring
                            return [dummyAstCreator_1.dummyBlockStatement(), context, paths, 'source does not allow destructuring'];
                        }
                        else if (isIrreducible(rhs)) {
                            const remainingBlockStatement = ast.blockStatement(otherStatements);
                            // force casting for weird errors
        ...
        BlockExpression(node, context, paths) {
            if (node.body.length === 0) {
                ...
                if (firstStatement.type === 'ReturnStatement') {
                    ...
                    if (kind !== 'const') {
                        // TODO: cannot use let or var
                        return [dummyAstCreator_1.dummyBlockExpression(), context, paths, 'cannot use let or var'];
                    }
                    else if (declarations.length <= 0 ||
                        declarations.length > 1 ||
                        declarations[0].type !== 'VariableDeclarator' ||
                        !declarations[0].init) {
                        // TODO: syntax error
                        return [dummyAstCreator_1.dummyBlockExpression(), context, paths, 'syntax error'];
                    }
                    else {
                        const declarator = declarations[0];
                        const rhs = declarator.init;
                        if (declarator.id.type !== 'Identifier') {
                            // TODO: source does not allow destructuring
                            return [dummyAstCreator_1.dummyBlockExpression(), context, paths, 'source does not allow destructuring'];
                        }
                        else if (isIrreducible(rhs)) {
                            const remainingBlockExpression = ast.blockExpression(otherStatements);
...
exports.codify = (node) => astring_1.generate(treeifyMain(node));
/**
...
exports.redexify = (node, path) => [
    astring_1.generate(pathifyMain(node, path)[0]),
    astring_1.generate(pathifyMain(node, path)[1])
];
exports.getRedex = (node, path) => pathifyMain(node, path)[1];
...
function substPredefinedFns(program, context) {
    if (context.prelude) {
        ...
        const listPreludeProgram = parser_1.parse(context.prelude, context);
