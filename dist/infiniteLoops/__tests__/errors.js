...
test('timeout errors are potential infinite loops', () => {
    ...
    expect(errors_2.isPotentialInfiniteLoop(error)).toBe(true);
});
...
test('other errors are not potential infinite loops', () => {
    ...
    expect(errors_2.isPotentialInfiniteLoop(runtimeError)).toBe(false);
    expect(errors_2.isPotentialInfiniteLoop(exceptionError)).toBe(false);
});
test('getInfiniteLoopData works when error is directly reported', () => {
    ...
    const result = errors_2.getInfiniteLoopData(context);
