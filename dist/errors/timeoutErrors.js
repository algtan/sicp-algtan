...
function getWarningMessage(maxExecTime) {
    ...
    return formatters_1.stripIndent `If you are certain your program is correct, press run again without editing your program.
...
class PotentialInfiniteLoopError extends TimeoutError {
    ...
    explain() {
        return formatters_1.stripIndent `${'Potential infinite loop detected'}.
...
class PotentialInfiniteRecursionError extends TimeoutError {
    ...
    explain() {
        const formattedCalls = this.calls.map(([executedName, executedArguments]) => `${executedName}(${executedArguments.map(arg => stringify_1.stringify(arg)).join(', ')})`);
        return formatters_1.stripIndent `${'Potential infinite recursion detected'}: ${formattedCalls.join(' ... ')}.
      ${getWarningMessage(this.maxExecTime)}`;
    }
    elaborate() {
        return this.explain();
