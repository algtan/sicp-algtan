...
class TypeError extends runtimeSourceError_1.RuntimeSourceError {
    constructor(node, side, expected, got, chapter = 4) {
        super(node);
        this.side = side;
...
exports.checkUnaryExpression = (node, operator, value, chapter = 4) => {
...
exports.checkBinaryExpression = (node, operator, chapter, left, right) => {
    switch (operator) {
        case '-':
        case '*':
        case '/':
...
exports.checkIfStatement = (node, test, chapter = 4) => {
    return isBool(test)
        ? undefined
        : new TypeError(node, ' as condition', 'boolean', typeOf(test), chapter);
};
exports.checkMemberAccess = (node, obj, prop) => {
    if (isObject(obj)) {
