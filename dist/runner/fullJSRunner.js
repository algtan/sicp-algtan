...
function parseFullJS(code, context) {
    let program;
    try {
        program = acorn_1.parse(code, FULL_JS_PARSER_OPTIONS);
    }
    catch (error) {
...
function fullJSRunner(code, context, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const preludeAndBuiltins = containsPrevEval(context)
            ? []
            : [...transpiler_1.getBuiltins(context.nativeStorage), ...preparePrelude(context)];
        // modules
        transpiler_1.hoistImportDeclarations(program);
        let modulePrefix;
        try {
            utils_1.appendModulesToContext(program, context);
            modulePrefix = transpiler_1.prefixModule(program);
        }
        catch (error) {
            context.errors.push(error instanceof runtimeSourceError_1.RuntimeSourceError ? error : yield errors_1.toSourceError(error));
            return utils_1.resolvedErrorPromise;
        }
        const preEvalProgram = create.program([
            ...preludeAndBuiltins,
            transpiler_1.evallerReplacer(create.identifier(constants_1.NATIVE_STORAGE_ID), new Set())
        ]);
        const preEvalCode = astring_1.generate(preEvalProgram) + modulePrefix;
        yield fullJSEval(preEvalCode, context.nativeStorage, options, context.moduleContexts);
        const { transpiled, sourceMapJson } = transpiler_1.transpile(program, context);
        try {
            ...
            context.errors.push(yield errors_1.toSourceError(error, sourceMapJson));
            return utils_1.resolvedErrorPromise;
        }
    });
}
