...
test('Transform single import decalration', () => {
    ...
    const result = transpiler_1.transformSingleImportDeclaration(123, program.body[0]);
...
test('Transform import decalrations variable decalarations', () => {
  ...
  `;
    const context = context_1.mockContext(4);
    const program = parser_1.parse(code, context);
    transpiler_1.transformImportDeclarations(program);
...
test('checkForUndefinedVariables accounts for import statements', () => {
  ...
  `;
    const context = context_1.mockContext(4);
    const program = parser_1.parse(code, context);
    transpiler_1.transpile(program, context, false);
});
