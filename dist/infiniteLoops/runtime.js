...
function checkForInfiniteLoopIfMeetsThreshold(stackPositions, state, functionName) {
    ...
    while (checkpoint <= stackPositions.length) {
        if (stackPositions.length === checkpoint) {
            detect_1.checkForInfiniteLoop(stackPositions, state, functionName);
        }
...
function testForInfiniteLoop(code, previousCodeStack) {
    const context = createContext_1.default(4, 'default', undefined, undefined);
    ...
    const instrumentedCode = instrument_1.instrument(previous, program, newBuiltins.keys());
