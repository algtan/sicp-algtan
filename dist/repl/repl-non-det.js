...
function _handleResult(result, context, callback) {
    if (result.status === 'finished' || result.status === 'suspended-non-det') {
        ...
        const error = new Error(index_1.parseError(context.errors));
...
function _resume(toResume, context, callback) {
    Promise.resolve(index_1.resume(toResume)).then((result) => {
...
function _run(cmd, context, options, callback) {
    if (cmd.trim() === constants_1.TRY_AGAIN) {
        ...
        index_1.runInContext(cmd, context, options).then(result => {
            _handleResult(result, context, callback);
        });
    }
}
function _startRepl(chapter = 1, useSubst, prelude = '') {
    // use defaults for everything
    const context = index_1.createContext(chapter, 'non-det');
    const options = {
    ...
    index_1.runInContext(prelude, context, options).then(preludeResult => {
        if (preludeResult.status === 'finished' || preludeResult.status === 'suspended-non-det') {
            ...
            throw new Error(index_1.parseError(context.errors));
        }
    });
}
function main() {
