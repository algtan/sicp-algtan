...
function parseAndTypeCheck(code, chapterOrContext = 1) {
    const context = typeof chapterOrContext === 'number' ? context_1.mockContext(chapterOrContext) : chapterOrContext;
    const program = parser_1.parse(code, context);
    expect(program).not.toBeUndefined();
    const validatedProgram = validator_1.validateAndAnnotate(program, context);
    return typeChecker_1.typeCheck(validatedProgram, context);
}
function topLevelTypesToString(program) {
    return program.body
        ...
        const type = actualNode.typability === 'Untypable'
            ? "Couldn't infer type"
            : stringify_1.typeToString(actualNode.type === 'FunctionDeclaration'
...
describe('type checking pairs and lists', () => {
    it('happy paths for list functions', () => {
        const context = context_1.mockContext(2);
    ...
    `);
        expect(index_1.parseError(errors1)).toMatchInlineSnapshot(`
    ...
    `;
      ...
      xs3: List<boolean>"
    `);
        expect(index_1.parseError(errors2)).toMatchInlineSnapshot(`""`);
    });
    ...
    `;
      ...
      ys: List<boolean>"
    `);
        expect(index_1.parseError(errors)).toMatchInlineSnapshot(`""`);
    });
    ...
    `);
        expect(index_1.parseError(errors, true)).toMatchInlineSnapshot(`
...
describe('type checking functions', () => {
    ...
    `;
        ...
        expect(index_1.parseError(errors)).toMatchInlineSnapshot(`"Line 3: foo contains cyclic reference to itself"`);
        expect(index_1.parseError(errors, true)).toMatchInlineSnapshot(`
...
describe('Type context from previous executions get saved', () => {
    it('source 1', () => {
      ...
      const f = x => x;
    `;
        const context = context_1.mockContext(1);
    ...
    `;
        const context = context_1.mockContext(2);
        yield index_1.runInContext('', context); // we run an empty program to simulate execution of prelude
    ...
    `;
        const context = context_1.mockContext(3);
        yield index_1.runInContext('', context); // we run an empty program to simulate execution of prelude
        const [program, errors] = parseAndTypeCheck(code1, context);
        expect(topLevelTypesToString(program)).toMatchInlineSnapshot(`
