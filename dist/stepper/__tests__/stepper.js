...
function getLastStepAsString(steps) {
    return stepper_1.codify(steps[steps.length - 1][0]).trim();
}
describe('Test codify works on non-circular abstract syntax graphs', () => {
    ...
    (1 + 2) * (3 + 4);
  `;
        const program = parser_1.parse(code, context_1.mockContext());
        expect(stepper_1.codify(program)).toMatchInlineSnapshot(`
      "(1 + 2) * (3 + 4);
      "
    `);
    });
  ...
  `; // not valid stepper input, but can be generated by stepper
        const program = parser_1.parse(code, context_1.mockContext(4));
        expect(stepper_1.codify(program)).toMatchInlineSnapshot(`
      "[1, 2];
      "
    `);
...
test('Test basic substitution', () => {
  ...
  `;
    const program = parser_1.parse(code, context_1.mockContext());
    const steps = stepper_1.getEvaluationSteps(program, context_1.mockContext(), 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchInlineSnapshot(`
    "(1 + 2) * (3 + 4);
...
test('Test binary operator error', () => {
    ...
    (1 + 2) * ('a' + 'string');
  `;
    const context = context_1.mockContext();
    const program = parser_1.parse(code, context);
    const steps = stepper_1.getEvaluationSteps(program, context, 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchInlineSnapshot(`
    "(1 + 2) * ('a' + 'string');
...
test('Test two statement substitution', () => {
  ...
  `;
    const program = parser_1.parse(code, context_1.mockContext());
    const steps = stepper_1.getEvaluationSteps(program, context_1.mockContext(4), 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchInlineSnapshot(`
    "(1 + 2) * (3 + 4);
    3 * 5;
...
test('subsets', () => {
 ...
 subsets(list(1, 2, 3));
  `;
    const program = parser_1.parse(code, context_1.mockContext(2));
    const steps = stepper_1.getEvaluationSteps(program, context_1.mockContext(2), 1000);
    expect(steps.map(x => stepper_1.codify(x[0])).join('\n')).toMatchSnapshot();
});
// source 0
