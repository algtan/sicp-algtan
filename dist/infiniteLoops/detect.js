...
function checkForCycle(stackPositions, state) {
    ...
    for (const item of transitions) {
        ...
        for (const transition of item) {
            ...
            innerStr.push(`(${instrument_1.getOriginalName(transition.name)}: ${stringifyCircular(transition.value)})`);
        }
...
function stringifyCircular(x) {
    ...
    const getCircularReplacer = () => {
        ...
        return (key, value) => {
            ...
            return symbolic_1.shallowConcretize(value);
        };
    };
...
function formatTransitionForMessage(transition, state) {
    ...
    if (symbolic === 'undefined') {
        // set as a constant
        return `${instrument_1.getOriginalName(transition.name)}' = ${transition.value}`;
    }
    else {
        const originalExpr = astring_1.generate(state.idToExprCache[transition.cachedSymbolicValue]);
        return `${instrument_1.getOriginalName(transition.name)}' = ${originalExpr}`;
    }
}
/**
...
function errorMessageMaker(ids, pathExprs, transitions, state) {
    return () => {
        ...
        ids.map(x => (x.name = instrument_1.getOriginalName(x.name)));
