...
function runInterpreter(program, context, options) {
    let it = interpreter_1.evaluate(program, context);
    let scheduler;
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            switch (context.variant) {
                case 'gpu':
                    gpu_1.transpileToGPU(program);
                    break;
                case 'lazy':
                    lazy_1.transpileToLazy(program);
                    break;
            }
            ;
            ({ transpiled, sourceMapJson } = transpiler_1.transpile(program, context));
            let value = yield evalContainer_1.sandboxedEval(transpiled, context.nativeStorage, options, context.moduleContexts);
            ...
            if (isDefaultVariant && errors_2.isPotentialInfiniteLoop(error)) {
                const detectedInfiniteLoop = runtime_1.testForInfiniteLoop(code, context.previousCode.slice(1));
...
function sourceRunner(code, context, verboseErrors, options = {}) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const isNativeRunnable = _1.determineExecutionMethod(theOptions, context, program, verboseErrors);
        if (isNativeRunnable && context.variant === 'native') {
            return yield fullJSRunner_1.fullJSRunner(code, context, theOptions);
        }
        // Handle preludes
