...
function* forceIt(val, context) {
    if (val instanceof Thunk) {
        if (val.isMemoized)
            return val.value;
        exports.pushEnvironment(context, val.env);
...
function* visit(context, node) {
    inspector_1.checkEditorBreakpoints(context, node);
...
function transformLogicalExpression(node) {
    if (node.operator === '&&') {
        return astCreator_1.conditionalExpression(node.left, node.right, astCreator_1.literal(false), node.loc);
    }
    else {
        return astCreator_1.conditionalExpression(node.left, astCreator_1.literal(true), node.right, node.loc);
    }
}
...
exports.evaluators = {
    ...
    UnaryExpression: function* (node, context) {
        ...
        return operators_1.evaluateUnaryExpression(node.operator, value);
    },
    BinaryExpression: function* (node, context) {
        ...
        return operators_1.evaluateBinaryExpression(node.operator, left, right);
    },
    ...
    ForStatement: function* (node, context) {
        ...
        const loopEnvironment = exports.createBlockEnvironment(context, 'forLoopEnvironment');
        exports.pushEnvironment(context, loopEnvironment);
        ...
        while (yield* actualValue(testNode, context)) {
            ...
            const environment = exports.createBlockEnvironment(context, 'forBlockEnvironment');
            exports.pushEnvironment(context, environment);
    ...
    BlockStatement: function* (node, context) {
        ...
        const environment = exports.createBlockEnvironment(context, 'blockEnvironment');
        exports.pushEnvironment(context, environment);
        ...
        try {
            ...
            if (!context.moduleContexts.has(moduleName)) {
                context.moduleContexts.set(moduleName, {
                    state: null,
                    tabs: moduleLoader_1.loadModuleTabs(moduleName, node)
                });
            }
            const functions = moduleLoader_1.loadModuleBundle(moduleName, context, node);
    ...
    Program: function* (node, context) {
        ...
        const environment = exports.createBlockEnvironment(context, 'programEnvironment');
        exports.pushEnvironment(context, environment);
...
function isEmptyEnvironment(env) {
    return lodash_1.isEmpty(env.head);
}
/**
...
function* evaluate(node, context) {
    ...
    if (result instanceof closure_1.default) {
        Object.defineProperty(getNonEmptyEnv(currentEnvironment(context)).head, lodash_1.uniqueId(), {
            value: result,
            writable: false,
            enumerable: true
        });
...
function* apply(context, fun, args, node, thisContext) {
    ...
    while (!(result instanceof ReturnValue)) {
        if (fun instanceof closure_1.default) {
            ...
            const bodyEnvironment = exports.createBlockEnvironment(context, 'functionBodyEnvironment');
            bodyEnvironment.thisContext = thisContext;
            exports.pushEnvironment(context, bodyEnvironment);
